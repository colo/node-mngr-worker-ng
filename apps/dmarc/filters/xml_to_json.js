...
module.exports = function(doc, opts, next, pipeline){
	...
	try {
		...
		const add_record = function(record){
			...
			template.ip = record.row.source_ip._text
			...
			let disposition = template.policy['disposition']
			if(report_template.records === undefined) report_template.records = {}
			if(!report_template.records[disposition]) report_template.records[disposition] = []
			// report_template.records[ip] = template
			report_template.records[disposition].push(template)
		}
