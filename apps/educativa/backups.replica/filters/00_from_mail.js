...
let debug = require('debug')('Server:Apps:Educativa:Backups.replica:Filters:00_from_mail');
let debug_internals = require('debug')('Server:Apps:Educativa:Backups.replica:Filters:00_from_mail:Internals');
...
module.exports = function(payload){
  ...
  let filter = function(doc, opts, next, pipeline){
    let { id, req, type, input } = opts
		debug('3rd filter', doc.text, opts)
		// process.exit(1)
		if(/^backups/.test(req.id) && req.type === 'fetch' && doc.text){
			dirvish(doc.text, opts, function(report){
				if(report !== undefined){
