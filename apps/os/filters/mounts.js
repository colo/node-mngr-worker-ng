	...
	if(
		...
		Array.each(val, function(_doc){
			...
			if(
				...
				**/
				next(
					{data: blocks, metadata: {host: host, path: module+'.['+mount_point+'].blocks', tag: ['os', 'mounts', mount_point, 'blocks'].combine(Object.values(info).combine(Object.keys(blocks)))}},
					opts,
					next,
					pipeline
				)
		...
		if(Object.getLength(used_doc_data) > 0){
			next(
				{data: used_doc_data, metadata: {host: host, path: module+'.used', tag: ['os', 'mounts', 'used', 'percentage'].combine(Object.values(used_doc_data_info)).combine(Object.values(used_doc_data))}},
				opts,
				next,
				pipeline
			)
