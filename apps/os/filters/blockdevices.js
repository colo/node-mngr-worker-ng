	...
	if(
		...
		const process_device = function(_doc, device){
			if(_doc.stats){
				...
				next(
					{data: requests, metadata: {host: host, path: module+'.'+device+'.requests', tag: ['os', 'blockdevices', device, 'requests'].combine(Object.keys(requests))}},
					opts,
					next,
					pipeline
				)
				next(
					{data: time, metadata: {host: host, path: module+'.'+device+'.time', tag: ['os', 'blockdevices', device, 'time'].combine(Object.keys(time))}},
					opts,
					next,
					pipeline
				)
				next(
					{data: sectors, metadata: {host: host, path: module+'.'+device+'.sectors', tag: ['os', 'blockdevices', device, 'sectors'].combine(Object.keys(sectors))}},
					opts,
					next,
					pipeline
				)
