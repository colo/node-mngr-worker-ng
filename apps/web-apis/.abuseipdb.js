...
let debug = require('debug')('Server:Apps:Web-Apis:AbuseIPDB:Pipeline');
let debug_internals = require('debug')('Server:Apps:Web-Apis:AbuseIPDB:Pipeline:Internals');
    ...
    filters: [
          ...
          Array.each(doc.data, function(group){
            if(group.tags && group.tags.contains('nginx') && group.tags.contains('enabled')){
              ...
              // Array.each(hosts, function(host){
                ...
                pipeline.get_input_by_id('input.hosts').fireEvent('onRange', {
                  // id: "once",
                  id: "range",
      ...
      function(doc, opts, next, pipeline){
        ...
        if(doc && doc.data && doc.metadata && doc.metadata.from === 'hosts'){
          ...
          pipeline.get_input_by_id('input.hosts').fireEvent('onSuspend')
            ...
            try{
              if(!forks[host]){
                forks[host] = fork(process.cwd()+'/apps/educativa/checks/libs/fork_filter', [
                  path.join(process.cwd(), '/apps/educativa/checks/filters/hosts'),
                ])
              ...
              if( err ) {
                ...
                // //   pipeline.get_input_by_id('input.hosts').fireEvent('onResume')
                // // }
      ...
      {
  			rethinkdb: {
  				id: "output.educativa.checks.hosts.rethinkdb",
  				conn: [
            output
  				],
