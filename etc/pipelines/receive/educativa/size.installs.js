...
const filters = [
  ...
  require(path.join(process.cwd(), 'apps/educativa/size/filters/csv')),
	require(path.join(process.cwd(), 'apps/educativa/size/filters/installs'))
]
...
const debug = require('debug')('pipelines:receive:educativa:size.installs:output')
...
let pipelines = [
  ...
  require(path.join(process.cwd(), 'apps/http-receiver/pipeline'))(
    {
      input: {
        id: "input.educativa.size.installs.http-receiver",
      ...
      output: [
        ...
        new JSPipelineOutput(Object.merge({
          id: "output.rethinkdb.educativa.size.installs",
