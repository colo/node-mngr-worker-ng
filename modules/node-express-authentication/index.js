const debug = require('debug')('node-express-authentication')
...
// module.exports =  new Class({
  ...
  // initialize: function(store, auth, options){
  ...
  _check_authentication: function(req, res, next){
		// console.log('---check_authentication--');
		// debug('_check_authentication', req)
		// process.exit(1)
		...
		//if(req.headers.authorization && req.headers.authorization.indexOf('Basic') == 0){
			...
			this._authenticate(req, res, next,  function(err, user, info) {
				...
				if (!user) {
					const message = (info) ? info.message : err;
					if(typeof this.app.log === 'function') this.app.log('login', 'warn', 'login authenticate ' + message);
					req.flash('error', err);
					this.app['403'](req, res, next, {error: message });
				}
				else{
					req.logIn(user, function(err) {
						if (err) {
							if(typeof this.app.log === 'function') this.app.log('login', 'error', err);
							req.flash('error', err);
							this.app['500'](req, res, next, { error: err.message });
						}
						else{
