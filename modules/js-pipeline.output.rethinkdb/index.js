...
module.exports = new Class({
  ...
  _save_docs: function(doc){
		debug_internals('_save_docs %o %s %o', doc, this.options.insert);
    // process.exit(1)
    // let db = this.options.conn[index].db
    // let table = this.options.conn[index].table
    // let conn = this.conns[index]

    this.r.table(this.options.table).insert(doc, this.options.insert).run(this.conn, function(err, result){
      debug_internals('insert result %o', err, result);
      this.fireEvent(this.ON_DOC_SAVED, [err, result])
    }.bind(this))
