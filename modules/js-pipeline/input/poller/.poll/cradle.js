...
const App = require('node-app-cradle-client');

var debug = require('debug')('Server:App:Pipeline:Input:Poller:Poll:Cradle');
var debug_events = require('debug')('Server:App:Pipeline:Input:Poller:Poll:Cradle:Events');
var debug_internals = require('debug')('Server:App:Pipeline:Input:Poller:Poll:Cradle:Internals');
...
module.exports = new Class({
  ...
  //exists: function(err, resp, body, req){
	...
	initialize: function(options){
		...
		this.profile('cradle_init');//start profiling
		
		var first_connect = function(result){
			debug_internals('first_connect %o', result.response);
			
			//console.log('CradlePoll.ON_CONNECT');
			//console.log(result);
		...
		this.profile('cradle_init');//end profiling
		
		this.log('cradle', 'info', 'cradle started');
  },
  connect: function(){
		debug('this.connect');
